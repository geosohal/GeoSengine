...
#define CHECKERRORNOX {GLenum err = GL::glGetError(); if (err != GL_NO_ERROR) { fprintf(stderr, "OpenGL cerror (at line %d): \n", __LINE__); } }
...
{
	if (buffers[0] != 0) {
        GL::glDeleteBuffers(NUM_VBs, buffers);
	}
...
void AnimatedModel::LoadMesh(const string& filename)
{
    ...
    GL::glGenBuffers(NUM_VBs, buffers);
...
bool AnimatedModel::InitModelInfo(const aiScene* pScene, const string& filename)
{
    ...
    GL::glBindBuffer(GL_ARRAY_BUFFER, buffers[POS_VB]);
    ...
    GL::glBindBuffer(GL_ARRAY_BUFFER, buffers[TEXCOORD_VB]);
    ...
    GL::glBindBuffer(GL_ARRAY_BUFFER, buffers[NORMAL_VB]);
    ...
    GL::glBindBuffer(GL_ARRAY_BUFFER, buffers[BONE_VB]);
    GL::glBufferData(GL_ARRAY_BUFFER, sizeof(VertexSkins[0]) * VertexSkins.size(), &VertexSkins[0], GL_STATIC_DRAW);
    GL::glEnableVertexAttribArray(BONE_ID_LOCATION);
    GL::glVertexAttribIPointer(BONE_ID_LOCATION, 4, GL_INT, sizeof(SkinnedVertData), (const GLvoid*)0);

    GL::glEnableVertexAttribArray(BONE_WEIGHT_LOCATION);
    GL::glVertexAttribPointer(BONE_WEIGHT_LOCATION, 4, GL_FLOAT, GL_FALSE, sizeof(SkinnedVertData), (const GLvoid*)16);

    GL::glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buffers[INDEX_BUFFER]);
    ...
    for (unsigned i = 0; i < scene->mNumAnimations; i++)
	{
        ...
        for (unsigned j = 0; j < anim->mNumChannels; j++)
		{
            ...
            for (unsigned k = 0; k < chan->mNumPositionKeys; k++)
			{
                ...
                boneAnim.AnimTrack.KeyFrames.push_back(key);
			}
...
void AnimatedModel::LoadBones(unsigned int meshIndex, vector<SkinnedVertData>& bones, const aiMesh* pMesh)
{
    for (unsigned i = 0; i < pMesh->mNumBones; i++)
	{
        ...
        for (unsigned k = 0; k < pMesh->mBones[i]->mNumWeights; k++)
		{
...
void AnimatedModel::DrawIKpoiints(int shaderId, BasicMesh* sphere, MAT4& VPmatrix)
{
    ...
    int loc = GL::glGetUniformLocation(shaderId, "custColor");
    GL::glUniform4fv(loc, 1, &custColor[0]);;

    for (unsigned i = 0; i < IKbetweens.size(); i++)
	{
		MAT4 t = Translate(IKbetweens[i].x, IKbetweens[i].y, IKbetweens[i].z)* Scale(.05f, .05f, .05f);
        loc = GL::glGetUniformLocation(shaderId, "gWVP");
        GL::glUniformMatrix4fv(loc, 1, GL_TRUE, (VPmatrix*t).Pntr());
		sphere->Render();
	}
	custColor = vec4(0, .7, .1, .1);
    loc = GL::glGetUniformLocation(shaderId, "custColor");
    GL::glUniform4fv(loc, 1, &custColor[0]);
    ...
    for (unsigned i = 0; i < 20; i++)
	{
		if (skeleton[i].isIK)	// dont draw IK bones
			continue;
        for (unsigned ch = 0; ch < skeleton[i].childrenIndices.size(); ch++)
		{
...
//                GL::glUniform1fARB(loc, 0.f);
//			else
//                GL::glUniform1fARB(loc, 1.f);
...
void AnimatedModel::UpdateAnimations(float timeSinceUpdate)
{
	...
	if (elapsedAnimTime > animTime)	// if we have finished the loop, we start elapsedtime over again at 0
		elapsedAnimTime = 0;
    VQS temp;
    UpdateAnimations(0, temp, vec3(0, 0, 0));
...
void AnimatedModel::UpdateAnimations(int currBone, VQS& currVQS, vec3 lastBonePos)
{
    ...
    int numKeys = skeleton[currBone].animationMap[currAnimation].AnimTrack.KeyFrames.size();
	if (numKeys == 1)	// no need to interpolate just use transform of the one key that there is
	{
        interpolatedTrans = skeleton[currBone].animationMap[currAnimation].AnimTrack.KeyFrames[0].KeyVQS;
	}
	else if (numKeys > 0)
	{
        ...
        float prevKeyTime = skeleton[currBone].animationMap[currAnimation].AnimTrack.KeyFrames[prevKeyIndex].Time;
        float nextKeyTime = skeleton[currBone].animationMap[currAnimation].AnimTrack.KeyFrames[nextKeyIndex].Time;
		...
		//
        if (skeleton[currBone].animationMap[currAnimation].AnimTrack.KeyFrames.size() > 0)
			interpolatedTrans = VQS::Interpolate(
                skeleton[currBone].animationMap[currAnimation].AnimTrack.KeyFrames[prevKeyIndex].KeyVQS,
                skeleton[currBone].animationMap[currAnimation].AnimTrack.KeyFrames[nextKeyIndex].KeyVQS,
				percentBetween);
	}
    ...
    for (unsigned i = 0; i < skeleton[currBone].childrenIndices.size(); i++)
	{
...
void AnimatedModel::UpdateIK(float timeSinceUpdate)
{
    ...
    for (unsigned parentIndex = skeleton[currJoint].parentIndex; parentIndex != 0; )
	{
...
void AnimatedModel::SetupBoneSpacePos(int rootIndex, MAT4 ikRot, int childToSkip)
{
    ...
    for (unsigned ch = 0; ch < skeleton[rootIndex].childrenIndices.size(); ch++)
	{
		int childIndex = skeleton[rootIndex].childrenIndices[ch];
		if (childIndex != childToSkip)
